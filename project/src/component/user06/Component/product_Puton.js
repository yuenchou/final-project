...
class ProductPuton extends Component {
    ...
    submitNewProduct = async (e) => {
        ...
        for (var i=0; i<upload.length; i++) {
            ...
            var imgData = this.state.productItem.productimg
        ...
        JSON.stringify(this.state.productItem),
        {headers: {'Content-Type': 'application/json'}})
        .then((response) => {
            console.log(response);
            this.setState({loading: true})
            window.location.href = `http://localhost:8000/VGT/memberPage/sellerInfo/tradeManage/${this.state.productItem.vgtid}`
        })
    };
